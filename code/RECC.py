...
class REC:
    ...
    def reset_epoch(self):
        ...
        if self.params_EST['R']:
            ...
            dKrr_sparse = self.kern.dK_dX_custom(Z)
    ...
    def run(self):
        ...
        for i_ep in range(self.nEpochs):
            ...
            print('epoch: ',i_ep)
            print('log marginal likelihood: ',self._log_marginal_likelihood)
    ...
    def inference(self, n0, C0, P0, log_marginal_likelihood0, log_Det_C0, dn_dR, dC_dR, dψ_dR, dn_dσ02, dC_dσ02, dψ_dσ02, dn_dl, dC_dl, dψ_dl, dn_dσn2, dC_dσn2, dψ_dσn2, X, Y):
        ...
        if α > 0:
            ...
            # VFE: α→0，v 退化为纯噪声，a 退化为 Tr[D_X]/(2σn²)
            v = np.full_like(d, σ_n2)
            a = np.sum(d) / (2 * σ_n2)        
        ...
        if α > 0:
            ...
            # VFE: v/α 项消失，但 a 的梯度路径变了
            dL_dv = - ( np.sum(H*B_,1) + (r-np.dot(H,γ_))**2 ) / (v**2)        
        # dL_dv = - ( np.sum(H*B_,1) -v/α + (r-np.dot(H,γ_))**2 ) / (v**2)
        ...
        if self.params_EST['R']:
            ...
            dKrr_sparse = self.kern.dK_dX_custom(Z)#, dK_dR=dKrr_sparse)
            # dKxr_sparse = np.zeros((B,J,D))
            dKxr_sparse = self.kern.dK_dX_custom(X, Z)#, dK_dR=dKxr_sparse)
